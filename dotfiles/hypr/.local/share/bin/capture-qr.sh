...
#
# Ported from Omarchy 4's omarchy-capture-qr.
...
if [[ -z $RESULT ]]; then
  "$(dirname "$0")"/notification-send.sh -g 󰐲 -u critical "No QR code found" "Select a region containing a QR code"
  exit 1
fi
...
"$(dirname "$0")"/notification-send.sh -g 󰐲 "QR code copied to clipboard"
